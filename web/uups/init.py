...
# init logger
logger = logging.getLogger("uasweb")
formatter = logging.Formatter('%(name)-12s %(asctime)s %(levelname)-8s %(message)s')
file_handler = logging.FileHandler(conf.uasconf.LOGPATH + "/uasweb.log")
...
logging.basicConfig(filename = conf.uasconf.LOGPATH + "/uasweb.log", format='%(name)-12s %(asctime)s %(levelname)-8s %(message)s', level=logging.DEBUG)
